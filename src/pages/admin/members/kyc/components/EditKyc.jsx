...
const EditDialog = ({ title, open, selectedId, onClose, fetchData }) => {
  const { enqueueSnackbar } = useSnackbar();
  const ReviewSchema = Yup.object().shape({
    kyc_identity_proof: Yup.mixed().test(
      "isFile",
      "Kyc identity proof is required",
  ...
  const onSubmit = async (inputData) => {
    ...
    reqData.append("kyc_identity_proof", inputData.kyc_identity_proof[0]);
    try {
      const { status, data } = await axiosInstance.post(
        `/api/admin-kyc-details-id`,
        reqData
      );
      if (status === 200) {
